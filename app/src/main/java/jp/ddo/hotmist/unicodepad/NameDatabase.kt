...
class NameDatabase(context: Context) {
    ...
    operator fun get(code: Int, column: String): String? {
        if (column == "name") {
            ...
            if (code in 0x3400..0x4DBF || code in 0x4E00..0x9FFF || code in 0x20000..0x2A6DF || code in 0x2A700..0x2B81E || code in 0x2B820..0x2CEAD || code in 0x2CEB0..0x2EBE0 || code in 0x2EBF0..0x2EE5D || code in 0x30000..0x3134A || code in 0x31350 .. 0x33479) return "CJK Unified Ideograph"
            if (code in 0xAC00..0xD7A3) return "Hangul Syllable"
            if (code in 0x17000..0x187FF) return "Tangut Ideograph"
            if (code in 0x18D00..0x18D20) return "Tangut Ideograph"
            if (code in 0x18E00..0x19191) return "Jurchen Character"
            if (code in 0x3D000..0x3FC3F) return "Small Seal Character"
        }
    ...
    fun getInt(code: Int, column: String): Int {
        if (column == "version") {
            ...
            if (code == 0x2B81E || code in 0x18D1F..0x18D20 || code in 0x18E00..0x19191 || code in 0x3D000..0x3FC3F) return 1800
        }
            ...
            return try {
                ...
                try {
                    db.rawQuery("SELECT * FROM version_code;", null).use { cur ->
                        cur.moveToFirst()
                        if (cur.getInt(0) != 75) throw SQLiteException()
                    }
                } catch (e: SQLiteException) {
                    db.close()
                    throw e
